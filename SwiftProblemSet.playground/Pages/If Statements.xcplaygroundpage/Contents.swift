...
func checkForRelease(bugs: Int, music: Bool, levels: Int) {
    ...
    if bugs > 10 {
        print("Need to extinguish some bugs!")
    }
    
    if !music {
        print("Add some tunes man!")
    }
    
    if levels < 5 {
        print("Must add more levels.")
    }
...
func checkTrainingStatus(name: String, bike: Bool, run: Bool) {
    if canFinishRun && canFinishBike {
        ...
        print("\(name) needs to work on biking.")
    } else if !canFinishRun {
        print("\(name) needs to work on running.")
    } else  {
        print("\(name) should really train more overall.")
    }
}
